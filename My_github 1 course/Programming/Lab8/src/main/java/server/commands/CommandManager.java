...
public class CommandManager {
     ...
     */
    public CommandResult execute(String name, Object argument, CollectionManager manager, int userId, PersonDAO personDAO) {
        Command command = commands.get(name);
        if (command == null) {
            return new CommandResult(false, "Неизвестная команда", false);
        }
        return command.execute(manager, argument, userId, personDAO);
    }
}
