...
public class Golem extends Monster{
    ...
    public void base_attk(Creature target) {
        System.out.println(this.name + " Прыгает и наносит урон по области " + target.getName() + "!!!");
        target.TakeDamage(this.attack_power);
    }

    public void base_attk_2stage(Creature target) {
        System.out.println(this.name + " Прыгает и наносит урон по области дважды " + target.getName() + "!!!");
        target.TakeDamage(this.attack_power*2);
    }

    public  void ramming(Creature target) {
        System.out.println(this.name + " Бьет своими кулаками " + target.getName() + "!!");
        target.TakeDamage(this.attack_power);
    }

    public  void ramming_2stage(Creature target) {
        System.out.println(this.name + " Бьет своими кулаками очень быстро " + target.getName() + "!!");
        target.TakeDamage(this.attack_power*2);
    }
