...
public class The_Planter extends Monster{
    String RED = "\u001B[31m";
    String RESET = "\u001B[0m";
    private boolean isSecondStageActivated = false;

    public The_Planter() {
        super("Плантера", 400, 40);
    }
    ...
    public  void ramming(Creature target) {
        System.out.println(this.name + " Стреляет по вам своими ядовитыми спорами " + target.getName() + "!!");
        target.TakeDamage(this.attack_power);
    }

    public  void ramming_2stage(Creature target) {
        System.out.println(this.name + " Пытается проглотить вас " + target.getName() + "!!");
        target.TakeDamage(this.attack_power);
    }
